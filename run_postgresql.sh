...
# -DuseBranch=f11106
xvfb-run -a openbox --startup "mvn -V clean install --settings=$PWD/postgresql/settings.xml -f $PWD/postgresql/pom.xml" 2>&1 | tee `basename --suffix=.sh $0`.log; openbox --exit
